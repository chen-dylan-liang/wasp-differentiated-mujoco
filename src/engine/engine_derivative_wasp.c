...
// F_hat = D * Delta_X
static void waspUpdate(const mjWASPBasis* basis, mjWASPCache *cache, int m, int n) {
    size_t i = cache->i;
    mju_mulMatMat(cache->DT, basis->C1 + i * n * n, cache->F_hat_T, n, n, m);
    mju_mulMatMatT(cache->tmp_n_m, basis->C2 + i * n, cache->fi, n, 1, m);
    mju_addToMat(cache->DT, cache->tmp_n_m, n, m);
    mju_mulMatTMat(cache->F_hat_T, basis->Delta_X, cache->DT, n, n, m);
    cache->i = (i + 1) % n;
}*/

static void waspSparseUpdate(const mjWASPBasis* basis, mjWASPCache *cache, int m, int n) {
    size_t i = cache->i;
    mjtNum* fi_old = cache->F_hat_T+i*m;
    mju_sub(cache->tmp_m, cache->fi, fi_old,m);
    mju_mulMatMatT(cache->tmp_n_m, basis->C2 + i * n, cache->tmp_m, n,1,m);
    mju_addToMat(cache->DT, cache->tmp_n_m, n, m);
    mju_copy(fi_old, cache->fi, m);
    cache->i = (i + 1) % n;
}
...
//     DsDa: (ns x na)
//     DsDu: (ns x nu)
int mjd_transitionWASP(const mjModel* m,
    ...
    if (A) {
        for (int i=0; i<ndx; i++)
            for (int j=0; j<ndx; j++) {
                if (j<nv) A[i*ndx+j] = (DyDq->DT)[j*ndx+i];
                else if (j<2*nv) A[i*ndx+j] = (DyDv->DT)[(j-nv)*ndx+i];
                else A[i*ndx+j]= (DyDa->DT)[(j-2*nv)*ndx+i];
            }
    }
    if (B) mju_transpose(B, DyDu->DT, nu, ndx);
    if (C) {
        for (int i=0; i<ns; i++)
            for (int j=0; j<ndx; j++) {
...
mjWASPBasis* mj_newWASPBasis(int n, mjtByte identity_basis) {
        mjWASPBasis* basis=(mjWASPBasis*)mju_malloc(sizeof(mjWASPBasis));
        basis->Delta_X = (mjtNum*) mju_malloc(sizeof(mjtNum)*n*n);
        basis->C1 = (mjtNum*) mju_malloc(sizeof(mjtNum)*n*n*n);
        basis->C2 = (mjtNum*) mju_malloc(sizeof(mjtNum)*n*n);
        if (identity_basis) {
            mju_zero( basis->Delta_X, n * n);
            for (int i = 0; i < n; i++) { basis->Delta_X[i * n + i] = 1.0; }
        } else {
            ...
            mju_qrDecompose( basis->Delta_X, R, M, n, n);
            mju_free(M);
            mju_free(R);
        }
        // C2 = Delta_X^T
        mju_transpose( basis->C2, basis-> Delta_X, n, n);
        mjtNum *tmp = (mjtNum *) mju_malloc(n * n * sizeof(mjtNum));
        mjtNum *I = (mjtNum *) mju_malloc(n * n * sizeof(mjtNum));
        mju_transpose( basis->C2, basis->Delta_X, n, n);
        for (int i = 0; i < n; i++) {
            mju_mulMatMatT(tmp,  basis->C2 + i * n,  basis->C2 + i * n, n, 1, n);
            // set to identity
            mju_zero(I, n * n);
            for (int j = 0; j < n; j++) { I[j * n + j] = 1.0; }
            mju_subFrom(I, tmp, n * n);
            mju_mulMatMat( basis->C1 + i * n * n, I,  basis->Delta_X, n, n, n);
        }
        mju_free(tmp);
        mju_free(I);
    return basis;
...
// delete wasp basis
void mj_deleteWASPBasis(mjWASPBasis* basis) {
    if (basis) {
        if (basis->Delta_X) {
            mju_free(basis->Delta_X);
            basis->Delta_X = NULL;
        }
        if (basis->C1) {
            mju_free(basis->C1);
...
// allocate wasp cache
mjWASPCache *mj_newWASPCache(int n, int m) {
    mjWASPCache *cache = (mjWASPCache *) mju_malloc(sizeof(mjWASPCache));
    cache->F_hat_T = (mjtNum *) mju_malloc(n * m * sizeof(mjtNum));
    cache->DT = (mjtNum *) mju_malloc(n * m * sizeof(mjtNum));
    cache->tmp_n_m = (mjtNum *) mju_malloc(n * m * sizeof(mjtNum));
    cache->fi = (mjtNum *) mju_malloc(m * sizeof(mjtNum));
    cache->tmp_m = (mjtNum *) mju_malloc(m * sizeof(mjtNum));
...
void mj_zeroWASPCache(mjWASPCache *cache, int n, int m) {
    ...
    mju_zero(cache->DT, n * m);
    mju_zero(cache->tmp_n_m, n * m);
    mju_zero(cache->fi, m);
    mju_zero(cache->tmp_m, m);
}
...
void mj_copyWASPCache(mjWASPCache *dest, const mjWASPCache* src, int n, int m) {
    ...
    mju_copy(dest->tmp_n_m, src->tmp_n_m, n * m);
    mju_copy(dest->fi, src->fi,m);
    mju_copy(dest->tmp_m, src->tmp_m,m);
}
